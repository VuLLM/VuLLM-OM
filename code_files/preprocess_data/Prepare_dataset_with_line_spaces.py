...
# get train data
def get_train(path_trainset, full_vulgen=False, drop_index_path=None):
    """
...
def get_test(dataset_path, full_vulgen=False, drop_index_path=None):
    """
...
def create_datasets(tokenizer, path_trainset, path_testset, full_vulgen=False, drop_indexes_train_path=None,drop_indexes_test_path=None):
    """
    ...
    """
    train = get_train(path_trainset, full_vulgen, drop_indexes_train_path)
    test = get_test(path_testset, full_vulgen, drop_indexes_test_path)
...
def get_testset_for_eval(tokenizer, path_testset, all_vulgen, drop_index_path):
    test = get_test(path_testset, full_vulgen=all_vulgen, drop_index_path=drop_index_path)
    test_edits = get_edits(test)
    test['inputs'] = get_inputs(test)
    test['outputs'] = get_outpus(test, test_edits)
    tokenized_test = tokenize(test, tokenizer)
...
def get_testset_for_replace(dataset_path, all_vulgen, delete_indexes_path):
    test = get_test(dataset_path, all_vulgen, delete_indexes_path)
    test_edits = get_edits(test)
    test['inputs'] = get_inputs(test)
    test['outputs'] = get_outpus(test, test_edits)
    return test
...
# train, test = create_datasets(AutoTokenizer.from_pretrained('Salesforce/codet5p-6b'), "Datasets/vulgen_train_with_diff_lines_spaces.csv", "Datasets/vulgen_test_with_diff_lines_spaces.csv",
#                               True, "pickle_files/location_with_spaces_to_delete_train.pkl", "pickle_files/location_with_spaces_to_delete_test.pkl")
