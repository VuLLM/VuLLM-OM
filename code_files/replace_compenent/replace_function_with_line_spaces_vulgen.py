...
from code_files import Prepare_dataset_with_line_spaces
...
def get_data(path_testset, path_res_mod, all_vulgen):
    ...
    nonvul = []
    real_modifications = []
    inject_turth = Prepare_dataset_with_line_spaces.get_testset_for_replace(path_testset, all_vulgen, 'pickle_files/location_with_spaces_to_delete_test.pkl')
    ...
    res_mod = None
    # res_mod = pd.read_csv(path_res_mod)
    # res_mod = res_mod['res_mod']
...
def inject_vul(nonvul, locations, injections):
    ...
    sum_added_rows = 0
    i = 0
    while i < len(injections):
        if injections[i][0] == "A":
            index_to_add = get_num_row(injections[i], original_lines)
            if index_to_add == -1:
                raise Error("Error: Could not find row to add below it.") 
            original_lines, i, sum_added_rows = add_rows(injections, i, index_to_add, original_lines, sum_added_rows)
            continue
        i += 1
    injections = delete_added_rows(injections)
    ...
    while i < len(mod_lines) and i < len(mod_lines):
        if mod_lines[i] == "<s>":
            if mod_lines[i+1][0] != "A":
                ...
                injections.append(mod_lines[i+1])
                i += 2
        # i += 1
    locations = '\n'.join(locations)
